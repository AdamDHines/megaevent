...
def _repr_kwargs(name, dt_ms):
    ...
    if name == "countmask":
        return {}                       # TODO(eventcv): fill in once countmask ships
    return {}
...
class EventStreamDataset(Dataset):
    ...
    def __init__(self, path, transform, representation, dt_ms, sensor,
    ...
    def _open(self):
        try:
            reader = eventcv.open(self.path, dt_ms=self.dt_ms, sensor_size=self.sensor,
                                  hot_pixel_filter=self.hot_pixel)
            if self.filter_dt_us:
...
# ---------------------------------------------------------------------------
@torch.no_grad()
def extract_descriptors(model, ds, device):
...
def descriptors_for(model, cfg, args, seq, device):
    ...
    t0 = time.time()
    desc = extract_descriptors(model, ds, device)
...
# 6. Matching + metrics
# ---------------------------------------------------------------------------
def load_gt(gt_path, nq, nr):
    """``<ref>_<query>_GT.npy`` stored ``[ref, query]`` -> bool ``[nq, nr]``.

    Event-LAB builds the band on a frame grid derived from the pose track, which need not
    match the number of event slices, so it is transposed to ``[query, ref]`` and resampled
    onto the descriptor grid with bilinear interpolation thresholded at ``>= 0.5``. That
    threshold is what preserves the band's density under a large resample.
    """
    gt = np.load(gt_path)
    g = (gt.T != 0).astype(np.float32)                  # -> [query, ref]
    if g.shape == (nq, nr):
        return g >= 0.5, gt.shape
    rs = sk_resize(g, (nq, nr), order=1, mode="edge", anti_aliasing=False, preserve_range=True)
    return rs >= 0.5, gt.shape


@torch.no_grad()
def sim_matrix(q_desc, ref_desc, device, chunk=512):
    """Full cosine-similarity ``[nq, nr]`` float32 on CPU (descriptors are L2-normalised)."""
    out = np.empty((q_desc.size(0), ref_desc.size(0)), dtype=np.float32)
    ref = ref_desc.to(device)
    for s in range(0, q_desc.size(0), chunk):
        e = min(s + chunk, q_desc.size(0))
        out[s:e] = (q_desc[s:e].to(device) @ ref.t()).float().cpu().numpy()
    del ref
    if device.type == "cuda":
...
def recalls_and_top1(sim, gt, ks=KS, chunk=512):
    """R@k over **all** queries, plus the top-1 reference index for each."""
    nq = sim.shape[0]
    maxk = min(max(ks), sim.shape[1])
    correct = {k: 0 for k in ks}
    top1 = np.empty(nq, dtype=np.int64)
    for s in range(0, nq, chunk):
        e = min(s + chunk, nq)
        idx = torch.from_numpy(sim[s:e]).topk(maxk, dim=1).indices.numpy()   # most similar
        top1[s:e] = idx[:, 0]
        hit = np.take_along_axis(gt[s:e], idx, axis=1)
        for k in ks:
            correct[k] += int(hit[:, :min(k, maxk)].any(axis=1).sum())
    return {k: correct[k] / nq for k in ks}, top1


# ---------------------------------------------------------------------------
# 7. Figure
# ---------------------------------------------------------------------------
# Sequential single-hue blue ramp, used REVERSED for distance so "0 distance = best match"
# reads as the darkest ink. TP/FP is blue vs orange (the green/red status pair is invisible
# to a colourblind reader), with marker shape as a second channel.
BLUE_RAMP = ["#cde2fb", "#b7d3f6", "#9ec5f4", "#86b6ef", "#6da7ec", "#5598e7",
             "#3987e5", "#2a78d6", "#256abf", "#1c5cab", "#184f95", "#104281", "#0d366b"]
CMAP_DIST = LinearSegmentedColormap.from_list("blue_r", BLUE_RAMP[::-1])
...
def make_figure(dist, gt, top1, tp, rec, ref, query, subtitle, out_png, gt_shape):
    nq, nr = dist.shape
    d_small = _block_reduce(dist, (1400, 1250), "min")            # min: keep the dark band
    g_small = _block_reduce(gt.astype(np.uint8), (1400, 1250), "max").astype(bool)

    fig, axes = plt.subplots(1, 2, figsize=(13.5, 5.8), constrained_layout=True)
    fig.suptitle(f"{ref} (reference) vs {query} (query) — all {nq} x {nr} pairs, {subtitle}",
                 fontsize=12, color=C_TEXT, y=1.04)

    ax = axes[0]
    im = ax.imshow(d_small, cmap=CMAP_DIST, aspect="auto", interpolation="nearest",
                   extent=[0, nr, nq, 0])
    ax.contour(np.linspace(0, nr, g_small.shape[1]), np.linspace(0, nq, g_small.shape[0]),
               g_small.astype(float), levels=[0.5], colors=[C_FP], linewidths=0.7)
    cb = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.02)
    cb.set_label("cosine distance  (dark = better match)", color=C_MUTED, fontsize=9)
    cb.ax.tick_params(colors=C_MUTED, labelsize=8)
    ax.set_title("(a) distance matrix + ground-truth band", fontsize=10, color=C_TEXT, loc="left")
    ax.set_xlabel(f"{ref} reference frame", color=C_MUTED, fontsize=9)
    ax.set_ylabel(f"{query} query frame", color=C_MUTED, fontsize=9)
    ...
    ax.imshow(g_small, cmap=LinearSegmentedColormap.from_list("gt", ["#ffffff", C_GT]),
              aspect="auto", interpolation="nearest", extent=[0, nr, nq, 0])
    qi = np.arange(nq)
    ax.scatter(top1[tp], qi[tp], s=1.6, c=C_TP, marker=".", linewidths=0,
               label=f"true positive (R@1 = {rec[KS[0]]:.3f})")
    ax.scatter(top1[~tp], qi[~tp], s=6.0, c=C_FP, marker="x", linewidths=0.5,
               label=f"false positive ({(~tp).sum()} of {nq})")
    ax.set_xlim(0, nr)
    ax.set_ylim(nq, 0)
    ax.set_title("(b) top-1 retrieval vs ground truth", fontsize=10, color=C_TEXT, loc="left")
    ax.set_xlabel(f"{ref} reference frame (retrieved)", color=C_MUTED, fontsize=9)
    ax.set_ylabel(f"{query} query frame", color=C_MUTED, fontsize=9)
    ...
    fig.text(0.5, -0.03, "   ".join(f"R@{k} {rec[k]:.3f}" for k in KS)
             + f"   |   GT {gt_shape[0]}x{gt_shape[1]} -> {nq}x{nr} (bilinear, thr>=0.5)"
...
# 8. Entry point
# ---------------------------------------------------------------------------
def evaluate(tag, subtitle, q_desc, ref_desc, gt, gt_shape, args, device, out_dir):
    """Score one descriptor space: similarity -> R@k -> saved matrix + figure."""
    t0 = time.time()
    suffix = "" if tag == "raw" else f"_{tag}"
    sim = sim_matrix(q_desc, ref_desc, device)
    rec, top1 = recalls_and_top1(sim, gt)
    tp = gt[np.arange(len(top1)), top1]

    np.subtract(1.0, sim, out=sim)                  # in-place: sim -> cosine distance
    np.save(os.path.join(out_dir, f"dist_{args.ref}_{args.query}{suffix}.npy"), sim)
    ...
    logger.info(f"[{tag}] " + "  ".join(f"R@{k}={rec[k]:.3f}" for k in KS)
                + f"   ({len(q_desc)}q x {len(ref_desc)}r, {q_desc.size(1)}-d, "
...
def run(args):
    ...
    ref_desc = descriptors_for(model, cfg, args, args.ref, device)
    q_desc = descriptors_for(model, cfg, args, args.query, device)

    gt, gt_shape = load_gt(ground_truth_path(args), len(q_desc), len(ref_desc))
    logger.info(f"ground truth {gt_shape[0]}x{gt_shape[1]} [ref, query] -> "
                f"{gt.shape[0]}x{gt.shape[1]} [query, ref], density {gt.mean():.4f}")

    out_dir = os.path.join(args.feature_dir, args.dataset)
    os.makedirs(out_dir, exist_ok=True)
    evaluate("raw", "no subsampling", q_desc, ref_desc, gt, gt_shape, args, device, out_dir)
    ...
    evaluate("pca", f"PCA-whitened to {pca['dim']}-d",
             pca_apply(q_desc, pca, device), pca_apply(ref_desc, pca, device),
