...
class FeatureAggregator(nn.Module):
    ...
    def __init__(
        ...
        self.token_features = nn.Sequential(
            nn.Linear(self.num_channels, self.mlp_dim),
            nn.ReLU(),
...
def load_megaloc_aggregator(aggregator, weights_path):
    ...
    print(
        f"[salad] warm-started SALAD head from MegaLoc {weights_path} "
        f"({len(sub)} tensors under {prefix})"
    )


def load_megaloc_projection(linear, weights_path):
    """Warm-start the post-SALAD projection from MegaLoc's ``aggregator.linear.*``.

    The companion to :func:`load_megaloc_aggregator`. MegaLoc's head is SALAD *plus* this
    layer — ``Linear(16640 -> 8448)``, 140.6M parameters, followed by an L2 norm — so
    warm-starting only the aggregator would hand a randomly initialised 140M-parameter map a
    head start it does not have. Loading both starts the whole head where MegaLoc's finished.

    Shapes are checked rather than trusted: a cluster_dim or desc_dim that does not match
    MegaLoc's geometry would otherwise load a transposed or truncated map in silence.
    """
    if weights_path is None:
        raise ValueError(
            "salad_proj with salad_init='megaloc' but no weights path — set "
            "GEPT_MEGALOC_WEIGHTS (or pass --megaloc-weights) to MegaLoc's "
            "model.safetensors (HF gberton/MegaLoc). Compute nodes are offline, so "
            "pre-download it on a login node."
        )
    from safetensors.torch import load_file

    full = load_file(weights_path)
    try:
        weight, bias = full["aggregator.linear.weight"], full["aggregator.linear.bias"]
    except KeyError as exc:
        raise KeyError(
            f"no {exc} in {weights_path} — not a MegaLoc checkpoint? (found e.g. {list(full)[:3]})"
        ) from exc
    if tuple(weight.shape) != tuple(linear.weight.shape):
        raise ValueError(
            f"MegaLoc's projection is {tuple(weight.shape)} but this model's is "
            f"{tuple(linear.weight.shape)}. The warm start needs MegaLoc's geometry: "
            f"--salad-init megaloc (cluster_dim 256 -> 16640) and salad_proj_dim 8448."
        )
    linear.load_state_dict({"weight": weight, "bias": bias}, strict=True)
    print(f"[salad] warm-started the {tuple(weight.shape)} projection from MegaLoc {weights_path}")
